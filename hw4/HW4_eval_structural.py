"""
eval_structural.py - Prompt-based × Structural-Only 驗證腳本

功能：與 eval.py 相同，但使用 Structural-Only prompt 格式
    （移除所有工具的 name、description、argument/result descriptions）

使用方式:
  python eval_structural.py              # 預設抽樣 50 筆
  python eval_structural.py 100          # 抽樣 100 筆
"""
...
def main():
    ...
    # ---- 初始化 log ----
    log_path = init_log("logs/eval_structural_log.jsonl")
    print(f"📝 API 呼叫記錄將寫入: {log_path}")
    print(f"🔧 Prompt 配置: Structural-Only")
    ...
    # ---- 結果報告 ----
    print("\n" + "=" * 60)
    print(f"📊 [Structural-Only] 驗證結果: {correct}/{total} = {correct/total*100:.2f}% 準確率")
    print("=" * 60)
